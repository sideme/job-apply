import { z } from "zod";
import {
  type CompanyModelRule,
  companyModelRulesSchema,
...
function serializeNullableString(
...
export const settingsRegistry = {
  ...
  whatsappEnabled: {
    ...
    default: (): boolean =>
      typeof process !== "undefined"
        ? parseBitBoolOrNull(process.env.WHATSAPP_ENABLED) === true
        : false,
  ...
  whatsappPhone: {
    ...
    schema: z
      ...
      .refine(
        ...
        "Enter a WhatsApp phone number including country code",
      ),
    default: (): string =>
      typeof process !== "undefined" ? process.env.CALLMEBOT_PHONE || "" : "",
    parse: parseNonEmptyStringOrNull,
    serialize: serializeNullableString,
  },
  ...
  semanticScoreWeight: {
    ...
    parse: (raw: string | undefined): number | null => {
      ...
      return Number.isNaN(parsed) ? null : Math.min(1, Math.max(0, parsed));
    },
    serialize: serializeNullableNumber,
  },
  embeddingEnabled: {
    ...
    default: (): boolean =>
      typeof process !== "undefined"
        ? parseBitBoolOrNull(process.env.EMBEDDING_ENABLED) === true
        : false,
    parse: parseBitBoolOrNull,
    serialize: serializeBitBool,
  },
  embeddingMaxJobsPerRun: {
    kind: "typed" as const,
    envKey: "EMBEDDING_MAX_JOBS_PER_RUN",
    schema: z.number().int().min(1).max(100),
    default: (): number => {
      const parsed = Number.parseInt(
        typeof process !== "undefined"
          ? process.env.EMBEDDING_MAX_JOBS_PER_RUN || "20"
          : "20",
        10,
      );
      return Number.isNaN(parsed) ? 20 : Math.min(100, Math.max(1, parsed));
    },
    parse: (raw: string | undefined): number | null => {
      if (!raw) return null;
      const parsed = Number.parseInt(raw, 10);
      return Number.isNaN(parsed) ? null : Math.min(100, Math.max(1, parsed));
    },
    serialize: serializeNullableNumber,
  },
  embeddingMaxInputChars: {
    kind: "typed" as const,
    envKey: "EMBEDDING_MAX_INPUT_CHARS",
    schema: z.number().int().min(1000).max(20000),
    default: (): number => {
      const parsed = Number.parseInt(
        typeof process !== "undefined"
          ? process.env.EMBEDDING_MAX_INPUT_CHARS || "6000"
          : "6000",
        10,
      );
      return Number.isNaN(parsed)
        ? 6000
        : Math.min(20000, Math.max(1000, parsed));
    },
    parse: (raw: string | undefined): number | null => {
      if (!raw) return null;
      const parsed = Number.parseInt(raw, 10);
  ...
  companyModelRules: {
    kind: "typed" as const,
    schema: companyModelRulesSchema,
    default: () => [],
    parse: parseCompanyModelRules,
    serialize: (value: CompanyModelRule[] | null | undefined) =>
