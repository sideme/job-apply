...
import { createApplicationFillSession } from "@server/services/application-fill-session";
import {
  deleteStageEvent,
  getStageEvents,
  getTasks,
...
import { enrichJobPostingDate } from "@server/services/posting-date-enrichment";
...
const applicationFillPlanRequestSchema = z.object({
...
/**
 * POST /api/jobs/:id/deep-analyze - opt-in LLM rationale for one reviewed job.
 * Pipeline scoring is local/embedding based; this deliberately consumes chat
 * tokens only when explicitly requested by the user.
 */
jobsRouter.post("/:id/deep-analyze", async (req: Request, res: Response) => {
  try {
    const chatKey =
      (await settingsRepo.getSetting("llmApiKey")) || process.env.LLM_API_KEY;
    if (!chatKey) {
      return fail(
        res,
        badRequest("Configure an LLM API key before running deep analysis."),
      );
    }
    ...
    logger.info("Completed LLM deep analysis", { jobId: job.id });
    ...
    const err =
      error instanceof AppError
        ...
        : new AppError({
            ...
            code: "INTERNAL_ERROR",
            message:
              error instanceof Error ? error.message : "Deep analysis failed",
          });
    return fail(res, err);
  }
});
...
/**
 * POST /api/jobs/:id/application-fill-plan - Resolve a captured form against
 * the local application-answer configuration. This route never submits a form.
 */
jobsRouter.post(
  "/:id/application-fill-plan",
  async (req: Request, res: Response) => {
    const parsed = applicationFillPlanRequestSchema.safeParse(req.body);
    if (!parsed.success) {
      return fail(
        res,
        badRequest("Invalid application form questions", {
    ...
    const plan = createApplicationFillPlan(job.id, parsed.data.questions);
    logger.info("Created application fill plan", {
      jobId: job.id,
      answerCount: plan.answers.length,
...
jobsRouter.post(
  "/:id/application-fill-session",
  asyncRoute(async (req: Request, res: Response) => {
    const job = await jobsRepo.getJobById(req.params.id);
    if (!job) throw notFound("Job not found");
    const session = createApplicationFillSession(job.id);
    logger.info("Created application auto-fill session", {
      jobId: job.id,
...
jobsRouter.post(
  "/:id/refresh-posting-date",
  asyncRoute(async (req: Request, res: Response) => {
    const job = await jobsRepo.getJobById(req.params.id);
    if (!job) throw notFound("Job not found");
    const enrichment = await enrichJobPostingDate(job);
    const updatedJob = await jobsRepo.getJobById(job.id);
    logger.info("Refreshed posting date from listing detail", {
      jobId: job.id,
      source: job.source,
      status: enrichment.status,
    });
    ok(res, { enrichment, job: updatedJob });
  }),
);
