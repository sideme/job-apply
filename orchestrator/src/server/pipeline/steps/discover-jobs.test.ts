...
describe("discoverJobsStep", () => {
  ...
  it("still trips the linkedin circuit breaker when a linkedin site_error is present but the overall run later hard-fails", async () => {
    const settingsRepo = await import("@server/repositories/settings");
    const registryModule = await import("@server/extractors/registry");
    const circuitBreaker = await import("../linkedin-circuit-breaker");
    const notifyWebhook = await import("./notify-webhook");
    ...
    const jobspyManifest = {
      ...
      run: vi.fn().mockResolvedValue({
        success: false,
        jobs: [],
        error: "failed to read output file for term 2",
        siteErrors: [{ site: "linkedin", error: "HTTPError: 429" }],
      }),
    };
