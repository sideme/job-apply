...
export const JobPage: React.FC = () => {
  ...
  const handleDeepAnalyze = async () => {
    await runAction("deep-analyze", async () => {
      if (!job) return;
      await api.deepAnalyzeJob(job.id);
  ...
  const currentStage = job
    ? (events.at(-1)?.toStage ??
      (job.status === "applied" || job.status === "in_progress"
        ...
        : null))
    : null;
  const canDeepAnalyze = Boolean(settingsQuery.data?.llmApiKeyHint);
  const keywordMissing = React.useMemo(() => {
    if (!job?.keywordMissing) return [];
    try {
      const values = JSON.parse(job.keywordMissing) as unknown;
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuItem
                    onSelect={() => void handleDeepAnalyze()}
                    disabled={!canDeepAnalyze || isBusy}
                  >
                    <Sparkles className="mr-2 h-4 w-4" />
                    Deep analysis {!canDeepAnalyze && "(API key required)"}
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
