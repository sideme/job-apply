...
const RUNTIME_PIPELINE_SOURCES = new Set<JobSource>([
  "indeed",
  "linkedin",
  "adzuna",
]);
...
export const orderedSources: JobSource[] = [...PIPELINE_EXTRACTOR_SOURCE_IDS]
  .filter((source) => RUNTIME_PIPELINE_SOURCES.has(source))
  .sort(
    (left, right) =>
      EXTRACTOR_SOURCE_METADATA[left].order -
      EXTRACTOR_SOURCE_METADATA[right].order,
  );
export const orderedFilterSources: JobSource[] = [...EXTRACTOR_SOURCE_IDS]
  .filter((source) => source !== "glassdoor" && source !== "ukvisajobs")
  .sort(
    (left, right) =>
      EXTRACTOR_SOURCE_METADATA[left].order -
      EXTRACTOR_SOURCE_METADATA[right].order,
