...
export async function deepAnalyzeJob(id: string): Promise<Job> {
  return fetchApi<Job>(`/jobs/${id}/deep-analyze`, { method: "POST" });
}
...
export async function getLocalResumeStatus(): Promise<LocalResumeStatus> {
  return fetchApi<LocalResumeStatus>("/settings/local-resume");
}

export async function uploadLocalResume(
  file: Blob,
...
export async function sendWhatsAppTest(): Promise<{ sent: true }> {
  return fetchApi<{ sent: true }>("/settings/whatsapp/test", {
    method: "POST",
  });
}

export async function createApplicationFillSession(
  jobId: string,
): Promise<{ code: string; expiresAt: string }> {
  return fetchApi<{ code: string; expiresAt: string }>(
    `/jobs/${encodeURIComponent(jobId)}/application-fill-session`,
    { method: "POST" },
  );
}

export async function refreshJobPostingDate(jobId: string): Promise<{
  enrichment: {
    status:
      | "updated"
      | "already_exact"
  ...
  return fetchApi(`/jobs/${encodeURIComponent(jobId)}/refresh-posting-date`, {
    method: "POST",
  });
}
